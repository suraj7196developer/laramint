...
// Reset cached roles and permissions
    ...
    $Themodels = [
        'department', 'course', 'session', 'term', 'file',
        'document', 'quiz', 'question', 'rubric', 'feedback',
        'participant', 'myCourse', 'plan', 'user', 'activity', 'badge', 'coinslog', 'configuration', 'documentfile', 'feedbackquestion', 'mentorcomment', 'permission', 'questiontype', 'quizquestion', 'role', 'sessionable', 'sessionterm', 'workout', 'workoutquizlog'
    ];
...
// create permissions
    ...
    foreach ($models as $model) {
        $role1->givePermissionTo($model . '.index');
        $role1->givePermissionTo($model . '.create');
        $role1->givePermissionTo($model . '.edit');
        $role1->givePermissionTo($model . '.delete');
        $role1->givePermissionTo($model . '.show');
    }
    $role1->givePermissionTo('document.order');
    $role1->givePermissionTo('menu.education');
    $role1->givePermissionTo('menu.toolbox');
    $role1->givePermissionTo('mentor.list');
